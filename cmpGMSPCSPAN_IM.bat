...
if exist %wrkdir%\%runid%SPAN_GMS_IM_DIFF.CSV del %wrkdir%\%runid%SPAN_GMS_IM_DIFF.CSV
if exist %wrkdir%\%runid%SPAN_GMS_IM_MATCHED.CSV del %wrkdir%\%runid%SPAN_GMS_IM_MATCHED.CSV
if exist %wrkdir%\GMSDIFF_IM.log del %wrkdir%\GMSDIFF_IM.log
perl %scriptDir%\ComparePCSPAN_GMS_IM.pl %runid% > %wrkdir%\GMSDIFF_IM.log
echo Comparing GMI results with PC Span .....
echo Generating mismatch report
echo Generating log file
echo .....................
set csvfile=%runid%SPAN_GMS_IM_DIFF.CSV
set csvfileAll=%runid%SPAN_GMS_IM_MATCHED.CSV
REM cscript.exe %scriptDir%\htmlGenerator.vbs %scriptDir% %csvfile% %runid%
REM set csvfile=%runid%CLCF2_DIFF.CSV
perl %scriptDir%\htmlGeneratorGMS_IM.pl %wrkdir% %csvfile% %runid%
perl %scriptDir%\htmlGeneratorGMS_IM.pl %wrkdir% %csvfileAll% %runid%
