...
%scriptDir%\marbat -pf export_ice_%runid%.csv -rf %exchfile% -of %runid%_results_ICE -wt 2000000 -wfprcap > %runid%_ICESPAN_LOG.log

:FILE_COMPARE
cd %scriptDir%
if exist %wrkdir%\TMP*.* del %wrkdir%\TMP*.*
if exist %wrkdir%\%runid%SPAN_CLCF2_DIFF.CSV del %wrkdir%\%runid%SPAN_CLCF2_DIFF.CSV
if exist %wrkdir%\%runid%SPAN_CLCF2_LOG.log del %wrkdir%\%runid%SPAN_CLCF2_LOG.log
perl %scriptDir%\CompareICESPAN_GMI_Detailed.pl %runid% > %wrkdir%\%runid%SPAN_CLCF2_LOG.log
