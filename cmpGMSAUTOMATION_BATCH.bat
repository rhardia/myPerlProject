...
set diffFile=diffFile_%$date%.htm
if exist %scriptDir%\%diffFile% del %scriptDir%\%diffFile%

Setlocal EnableDelayedExpansion

for /f "tokens=*" %%a in (exchList.txt) do (
echo Running test for %%a
REM set /p runid="Enter Exchange Method Code : "
set runid=%%a
call :PROCESSLINE
)
goto :PRINTHTML
...
type %wrkdir%\%runid%SPAN_GMS_DIFF.htm >> %scriptDir%\%diffFile%
REM cscript.exe %scriptDir%\htmlOpener.vbs %wrkdir% %csvfile%
cd %cwd%
REM cscript.exe %scriptDir%\htmlOpener.vbs %scriptDir% %csvfileAll%
REM cscript.exe %scriptDir%\htmlGenerator.vbs %scriptDir% %csvfile% %runid%
echo %runid% Completed Successfully
goto :END
:PRINTHTML
cscript.exe %scriptDir%\htmlOpener.vbs %scriptDir% %diffFile%
:END
