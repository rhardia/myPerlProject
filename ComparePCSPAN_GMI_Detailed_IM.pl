...
#			print "GMI Exchange $GMIExCode not found under downloaded GMIGMST1 file. Exchange $GMIExCode ignored from comparison ...\n";
	...
	$mgResults{$dbkey} = $GMIIM.",".$GMINOV.",".$GMIRISK.",".$GMIIMS.",".$GMIICS.",".$GMISOM;
}
close CSV;
################################################################################################################################################################################################
...
print WRITECSV "SPAN Method Code,Account,Combine Comm Code,Currency,SPAN Init Margin,SPAN NOV,SPAN RISK, SPAN IMS,SPAN ICS,SPAN SOM,GMI Init Margin,GMI NOV,GMI RISK, GMI IMS,GMI ICS,GMI SOM,Diff Init Margin,Diff NOV,Diff SOM \n";
print WRITECSVALL "SPAN Method Code,Account,Combine Comm Code,Currency,SPAN Init Margin,SPAN NOV,SPAN RISK, SPAN IMS,SPAN ICS,SPAN SOM,GMI Init Margin,GMI NOV,GMI RISK, GMI IMS,GMI ICS,GMI SOM,Diff Init Margin,Diff NOV,Diff SOM \n";
...
###########################################################################################################################################
	$SPANFIRM = $SpanFields[5];
	chomp($SPANFIRM);		
	$SPANFIRM =~ s/^\s+|\s+$//g;
	$strFind = " ";
	$strReplace = "";
	$SPANFIRM =~ s/$strFind/$strReplace/gi;

	if ($ACCODE ne $SPANFIRM) {
		$ACCODE=$SPANFIRM.$ACCODE;
	}
...
#	$COMB_EXCH =~ s/^\s+|\s+$//g;
	...
	if(exists $mgResults{$SPANKey}){
			...
			my $diffTotal = $COMB_INIT - $arrResults[0];
			...
			if ($diffTotal>10){
				print WRITECSV "$METHOD,$ACCODE,$COMB_CODE,$CURRCODE,$COMB_INIT,$NOV,$SCANRISK,$IMS,$ICS,$SOM,$arrResults[0],$arrResults[1],$arrResults[2],$arrResults[3],$arrResults[4],$arrResults[5],$diffTotal,$diffNOV,$diffSOM \n";
			}
			else{
				print WRITECSVALL "$METHOD,$ACCODE,$COMB_CODE,$CURRCODE,$COMB_INIT,$NOV,$SCANRISK,$IMS,$ICS,$SOM,$arrResults[0],$arrResults[1],$arrResults[2],$arrResults[3],$arrResults[4],$arrResults[5],$diffTotal,$diffNOV,$diffSOM \n";
			}
